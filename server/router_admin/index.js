module.exports = (app) => {
  ...
  function valueIsEmpty(obj) {
    let dd = Object.values(obj).includes("");
    return dd;
  }
  ...
  router.delete("/:id", verifyToken(), async (req, res) => {
    ...
    // delete historyModel._id //删除 id 项
  ...
  router.put("/:id", verifyToken(), async (req, res) => {
    ...
    if (valueIsEmpty(req.body)) {
      res.status(423).send({ message: "信息有误" });
    } else {
      res.send({
        message: "修改成功",
      });
  ...
  router.get("/", async (req, res) => {
    ...
    if (req.params.resource == "categorys") {
      ...
      items = await req.Model.find().sort({
        pickTimes: -1,
      });
    } else {
      items = await req.Model.find().populate("categoryID");
    }
    res.send(items);
  ...
  router.get("/position/:position/:usernameID?", async (req, res) => {
    ...
    const items = await req.Model.find(searchModel)
      .populate({
        path: "usernameID",
  ...
  app.post("/admin/api/testUsername", async (req, res) => {
    ...
    console.log(user);
    // assert(!user, 200, { message: "用户名已存在", flag: false });
  ...
  app.post("/admin/api/registGuest", async (req, res) => {
    let arr = [
      /^([A-Za-z0-9@._]){5,18}$/.test(req.body.username),
      /^([A-Za-z0-9@._]){5,18}$/.test(req.body.password),
    ];
  ...
  app.get("/admin/api/charts/member", async (req, res) => {
    const userList = await User.find();
    const idList = userList.map((item) => item._id);
    const nickList = userList.map((item) => item.nickname);


      const userfoods0 =await Food.find({usernameID:idList[0]});
      const userfoods1 =await Food.find({usernameID:idList[1]});
      const userfoods2 =await Food.find({usernameID:idList[2]});
      const userfoods3 =await Food.find({usernameID:idList[3]});
      const List = [userfoods0.length,
        userfoods1.length,
        userfoods2.length,
        userfoods3.length]
    let list = []
      for (let i = 0; i < nickList.length; i++) {
        const nickname = nickList[i];
    
       
        list.push({nickname:nickname,own:List[i]})
      }
